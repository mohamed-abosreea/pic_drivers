...
Err_check_t gpio_pin_get_direction(const pin_config_t *pin_config, direction_t *dir) {
    Err_check_t e_check = OK;
    if (((void*)0) == pin_config || ((void*)0) == dir) {
        e_check = N_OK;
    } else {
        *dir = ((*tris_registers[pin_config ->port] >> pin_config ->pin) & 1);
        e_check = OK;
    }
    return e_check;
}
...
Err_check_t gpio_pin_get_state(const pin_config_t *pin_config, state_t * state) {
    Err_check_t e_check = OK;
    if (((void*)0) == pin_config || ((void*)0) == state) {
        e_check = N_OK;
    } else {
        *state = ((*port_registers[pin_config -> port] >> pin_config->pin) & 1);
    }
    return e_check;
}
